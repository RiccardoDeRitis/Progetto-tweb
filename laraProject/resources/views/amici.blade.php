...
<div class="container_amici" style="width: 40%; margin-left: 9.6%; display: inline-block;">
    @foreach($amici as $amico)
        ...
        @if ($amico->Amicizia == 1)
            <div class="div_amici">
                @if ($amico->IDUtente == Auth::user()->id)
                    <h1 class="user_search_data" style="width: fit-content; display: inline-block;"> {{$utenti[$amico->IDUtenteAmico - 1]->Nome}} {{$utenti[$amico->IDUtenteAmico - 1]->Cognome}} </h1>
                ...
                @else
                    <h1 class="user_search_data" style="width: fit-content; display: inline-block;"> {{$utenti[$amico->IDUtente - 1]->Nome}} {{$utenti[$amico->IDUtente - 1]->Cognome}} </h1>
